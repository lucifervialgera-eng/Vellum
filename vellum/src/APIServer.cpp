...
void APIServer::setupRoutes() {
    CROW_ROUTE(app_, "/api/vm/create").methods("POST"_method)([this](const crow::request& req) {
    ...
    CROW_WEBSOCKET_ROUTE(app_, "/ws/console")
    .onopen([this](crow::websocket::connection& conn) {
        std::lock_guard<std::mutex> lock(connections_mutex_);
        console_connections_["default"].insert(&conn);
        std::cout << "Console WebSocket opened" << std::endl;
    })
    .onmessage([this](crow::websocket::connection& conn, const std::string& data, bool is_binary) {
        auto json = crow::json::load(data);
        if (json && json["type"] == "input" && json["vmId"].t() == crow::json::type::String) {
            std::string vm_id = json["vmId"].s();
...
void APIServer::broadcastConsoleOutput(const std::string& vm_id, const std::string& data) {
    std::lock_guard<std::mutex> lock(connections_mutex_);
    std::string msg = data;
    for (auto conn : console_connections_["default"]) {
        conn->send_text(msg);
    }
}
