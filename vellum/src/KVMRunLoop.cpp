...
void VMInstance::vcpuRunLoop(int vcpu_id) {
    ...
    struct kvm_run* run = static_cast<struct kvm_run*>(mmap(nullptr, KVM_RUN_SIZE, PROT_READ | PROT_WRITE,
...
void VMInstance::handleIO(struct kvm_run* run) {
    ...
    if (run->io.port == 0x3f8) {  // COM1 port
        if (run->io.direction == KVM_EXIT_IO_OUT) {
            // Output to console
            std::string output(reinterpret_cast<char*>(run->io.data), run->io.size);
